...
function unitEconomics(gmv: number, rate = RECOMMENDED_RATE, whiteboxPaysAcquiring = true) {
  const commission = gmv * rate;
  const invoice = partnerInvoice(gmv, rate);
  const acquiring = whiteboxPaysAcquiring ? gmv * ACQUIRING_RATE : 0;
...
const costRows = [
  ...
  ["Онлайн-касса / чеки / ОФД", "2 000 - 8 000 ₽", "Нужно, если WhiteBox принимает оплату от физлиц и формирует чек."],
...
const incomeRows = [
  ...
  ["Managed payments", "+2-3 п.п.", "Если WhiteBox сам тащит эквайринг, чеки, возвраты и payout-сверку."],
...
const taxRows = [
  ["УСН доходы", "6%", "Стартовый режим, если расходы небольшие и юридическая модель считает доходом именно комиссию WhiteBox."],
  ["УСН доходы-расходы", "15% с прибыли", "Может стать выгоднее, если WhiteBox сам оплачивает эквайринг, рекламу, поддержку и фонд оплаты труда."],
  ...
  ["54-ФЗ", "ККТ и электронные чеки", "Если деньги от физлиц принимает WhiteBox, нужно заранее спроектировать чеки и фискализацию."],
];
...
const recommendationCards = [
  ...
  {
    icon: CreditCard,
    title: "10% только если WhiteBox несёт платежи",
    ...
    >
      ...
      <section className="rounded-[2rem] border border-cyan-200/15 bg-[radial-gradient(circle_at_top_left,rgba(34,211,238,0.16),transparent_34%),rgba(2,6,23,0.82)] p-6">
        <div className="mb-5 flex flex-col justify-between gap-3 lg:flex-row lg:items-end">
          <div>
            ...
            <p className="mt-2 max-w-3xl text-sm leading-6 text-white/60">
              Так компания без подписок платит понятную абонентку, а компания с оборотом фактически получает доступ бесплатно: платит только комиссию, которая уже доказала ценность WhiteBox.
            </p>
          </div>
        ...
        <p className="mt-3 text-xs leading-5 text-white/45">
          * Net грубо считает УСН 6%, обслуживание партнёра и эквайринг 2.8%, если WhiteBox несёт его сам. Если эквайринг проходит как pass-through за счёт партнёра, маржа выше.
        </p>
      </section>
      ...
      <section className="grid gap-4 xl:grid-cols-[1.05fr_0.95fr]">
        <Card className="border-white/10 bg-slate-950/70 py-0">
          <CardContent className="p-6">
            ...
            <MiniTable
              columns={["Партнёры", "Средний GMV", "MRR WhiteBox", "После переменных"]}
            ...
            <div className="mb-5 flex items-start gap-3">
              ...
              <div>
                ...
                <p className="mt-1 text-sm leading-6 text-muted-foreground">
                  5% хороши как welcome-тариф на 60-90 дней. 7% — здоровая базовая ставка. 10% — только когда WhiteBox берёт на себя платежную операционку, фискализацию, возвраты, антифрод и сверку выплат.
                </p>
              </div>
            </div>
        ...
        <div className="mb-5 flex items-start gap-3">
          ...
          <div>
            ...
            <p className="mt-1 max-w-3xl text-sm leading-6 text-muted-foreground">
              Самое важное решение: WhiteBox должен быть агентом/платформой, где доход WhiteBox — комиссия, а не весь оборот подписок. Это надо оформить договором и проверить с бухгалтером, иначе налоговая база может стать болезненной.
            </p>
          </div>
        </div>
      ...
      <section className="grid gap-4 xl:grid-cols-3">
        {[
          [Landmark, "Модель А: агентская", "Рекомендуется. WhiteBox принимает/сопровождает подписки, удерживает комиссию, партнёру уходит остальное. Нужны договор, отчёты агента, корректная фискализация."],
          [CreditCard, "Модель B: платежи у партнёра", "Самая простая юридически: партнёр сам принимает деньги, WhiteBox выставляет ему SaaS/commission invoice. Минус: меньше контроля и хуже UX подписок."],
          [AlertTriangle, "Модель C: merchant of record", "Не для старта. WhiteBox продаёт всё от себя, несёт возвраты, чеки, НДС-риски и потребительскую ответственность. Комиссия ниже 10-12% опасна."],
        ].map(([Icon, title, text]) => {
      ...
      <section className="rounded-[2rem] border border-white/10 bg-slate-950 p-6">
        ...
        <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-4">
          {[
            ...
            ["Managed", "10%", "WhiteBox закрывает платежи, чеки, возвраты и сверку."],
      ...
      <section className="rounded-[2rem] border border-white/10 bg-white/[0.035] p-6">
        ...
        <p className="mt-5 text-xs leading-5 text-muted-foreground">
          Расчёты используют допущения: УСН 6% по доходу WhiteBox, эквайринг 2.8%, комиссия как агентское вознаграждение, без зарплаты основателя. Перед продом модель нужно подтвердить с бухгалтером, юристом и выбранным платёжным провайдером.
        </p>
      </section>
    </ConceptPageShell>
  );
