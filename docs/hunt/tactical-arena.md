...
`src/lib/hunt/tactics.ts` is a pure shared rules module. Client calculations only highlight legal actions; `POST /api/hunt/training` validates and resolves on the Next.js server. Signed, two-hour snapshots reject modified state. The signature is domain-separated from authentication tokens. `JWT_SECRET` provides a shared signing key across server instances; a process-local random key supports local development without it, invalidating sessions on server restart. Training grants no rewards, so replaying a signed snapshot is harmless and intentionally allowed. Competitive matchmaking will require durable matches and atomic turn submission before granting rewards.

The deterministic AI evaluates point income, contesting, threat, distance, lethal attacks and shielding. It uses the same movement, attack, bonus and exhaustion validators. Unit IDs and sides are explicit so the model can later be extended to multiple players per team. This version is player versus AI only.
